...
function showNotification(message, type = "info") {
  ...
  toast.className = `fixed bottom-4 right-4 px-6 py-3 rounded-lg font-bold text-white shadow-lg z-50 animate-slide-in-right ${
    type === "success"
      ? "bg-green-500"
      : type === "error"
      ? "bg-red-500"
    ...
    try {
      ...
      remindersList.innerHTML = data.reminders
        .map((reminder) => {
          ...
          const status =
            reminder.status === "active" ? "âœ… Active" : "â¸ï¸ Paused";
      ...
      return `
            <div class="contributor-item">
                ...
                <div class="contributor-info">
                    ...
                    <div class="contributor-stats">Level ${
                      contributor.level
                    }</div>
                </div>
...
function toggleDateInputType() {
  ...
  input.type = isDateOnly ? "date" : "datetime-local";
}
          ...
          <div class="flex-grow min-w-0">
            <div class="flex items-center gap-2 mb-1">
              <h4 class="font-bold truncate" title="${escapeHtml(
                reminder.message
              )}">${escapeHtml(reminder.message.substring(0, 50))}${
        reminder.message.length > 50 ? "..." : ""
          ...
          <div class="flex gap-2 ml-2">
            ...
            <button class="text-slate-500 hover:text-yellow-400 transition-colors" 
              ...
              <i class="fas fa-${
                reminder.status === "active" ? "pause" : "play"
              }"></i>
            </button>
...
async function saveReminder() {
  ...
  let startTime = document.getElementById("reminderDateTime").value;
...
function openReminderModal(reminderIdx = null) {
  ...
  document.getElementById("reminderChannel").value = "";
  document.getElementById("reminderRole").value = "";
  ...
  document.getElementById("reminderDateTime").value = localIso;
  ...
  if (isEdit) {
    ...
    if (r.next_run) {
      ...
      document.getElementById("reminderDateTime").value = iso;
    }
 ...
 */
async function initRestrictionTab(force = false) {
 ...
 * @param {string} guildId - Current guild ID
 */
async function loadRestrictions(guildId) {
  const container = document.getElementById("restrictionsList");
  if (!container) {
    console.error("restrictionsList container not found");
    return;
  }
  try {
    const res = await fetch(
 ...
 * @param {string} guildId - Current guild ID
 */
async function loadResDiscordData(guildId) {
  const res = await fetch(`/api/server/${guildId}/discord-data`);
  const data = await res.json();
...
/**
 * Extract the guild ID from the current URL.
 * Supports `/server/{guildId}` format or falls back to the last path segment.
 */
function getGuildIdFromUrl() {
  const match = window.location.href.match(/\/server\/(\d+)/);
  return match ? match[1] : window.location.pathname.split("/").pop();
 ...
 */
async function initYoutubeTab(force = false) {
...
async function saveYTConfig() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      showToast(
        `âœ… Channel ${action}! Seeded ${seededCount} video${
          seededCount !== 1 ? "s" : ""
        }.`,
        "success"
      );
      ...
      <tr>
        ...
        <td>
          <div class="flex items-center gap-3">
            ...
            <span class="font-bold ${rank <= 3 ? "" : "text-slate-300"}">${
          user.username || "Unknown User"
        }</span>
          </div>
        </td>
        <td class="text-center font-bold text-indigo-400">${
          user.level || 0
        }</td>
