...
This script:
- Starts the Flask application in a background daemon thread.
...
logger = logging.getLogger("run_localhost")
...
def start_flask_thread() -> None:
    """
    Start the Flask frontend. Imports the Flask module dynamically
    to rely on modified sys.path set earlier. Expects a callable
    `run_flask_app()` in the `app` module that starts the Flask server.
    """
    logger.info("Starting Flask server (port %s).", SERVER_PORT)
    try:
        app_module = importlib.import_module("app")
    except Exception:
        logger.exception("Failed to import Flask app module 'app'.")
        return

    run_flask: Optional[Callable[[], Any]] = getattr(app_module, "run_flask_app", None)
    if not callable(run_flask):
        logger.error("app.run_flask_app() not found or not callable.")
        return
...
if __name__ == "__main__":
    logger.info("Supporter Bot - threaded runner starting.")
